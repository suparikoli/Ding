...
frappe.ui.form.on('Contact Meet', {
    ...
    refresh: function(frm) {
        ...
        frm.add_custom_button(__('Get Directions'), function() {
            var coord = dingExtractLatLng(frm.doc.contact_location);
            if (!coord) {
                frappe.msgprint(__('Contact location is not set.'));
                return;
            }
        ...
        frm.add_custom_button(__('Update Contact Location'), function() {
            if (frm.doc.contact) {
                window.open('/app/contact/' + encodeURIComponent(frm.doc.contact), '_blank');
            } else {
                ...
                frm.add_custom_button(__('Create / Update Issue'), function() {
                    if (frm.doc.linked_doctype === 'Issue' && frm.doc.linked_name) {
                        ...
                        frappe.new_doc('Issue', {
                            subject: 'Service visit follow-up: ' + (frm.doc.contact || ''),
                            description: frm.doc.comment || ''
                        });
                    }
                }, __('Convert'));
            }
            frm.add_custom_button(__('Add Note to Contact'), function() {
                window.open('/app/contact/' + encodeURIComponent(frm.doc.contact), '_blank');
            }, __('Convert'));
        }
    }
