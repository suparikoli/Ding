...
class ContactMeet(Document):
	def before_insert(self):
		...
		if self.contact:
			if not self.contact_location:
				self.contact_location = frappe.db.get_value(
					"Contact", self.contact, "contact_geolocation"
				)
			if not self.contact_company:
				self.contact_company = frappe.db.get_value(
					"Contact", self.contact, "company_name"
				)
	...
	def validate(self):
		...
		target = parse_geo(self.contact_location)
