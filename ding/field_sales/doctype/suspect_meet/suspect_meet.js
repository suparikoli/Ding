...
frappe.ui.form.on('Suspect Meet', {
    onload: function(frm) {
    ...
    refresh: function(frm) {
        ...
        frm.add_custom_button(__('Get Directions'), function() {
            var suspectLocation = frm.doc.custom_suspect_geolocation;
            if (isValidLatLong(suspectLocation)) {
                var googleMapsUrl = 'https://www.google.com/maps?q=' + encodeURIComponent(suspectLocation);
                window.open(googleMapsUrl, '_blank');
            } else {
                frappe.msgprint(__('Invalid suspect Location.'));
            }
        });
        ...
        // Add "Update suspect Location" button
        frm.add_custom_button(__('Update suspect Location'), function() {
            var suspectLocation = frm.doc.custom_suspect_geolocation;
            if (!suspectLocation || !isValidLatLong(suspectLocation)) {
                var suspectName = frm.doc.suspect;
                if (suspectName) {
                    var suspectUrl = frappe.urllib.get_base_url() + '/app/suspect/' + suspectName;
                    window.open(suspectUrl, '_blank');
                } else {
                    frappe.msgprint(__('Please select a suspect.'));
                }
            }
        });
    },

    custom_suspect_geolocation: function(frm) {
...
function calculateAndSetDistance(frm) {
    ...
    if (suspectLocation && loggedGeoLocation) {
        var distance = calculateDistance(suspectLocation, loggedGeoLocation);
