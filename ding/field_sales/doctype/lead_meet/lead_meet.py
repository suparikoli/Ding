...
class LeadMeet(Document):
	def before_insert(self):
		if not self.creator:
			self.creator = frappe.session.user
		if not self.check_in_time:
			self.check_in_time = now_datetime()
		if self.lead and not self.lead_location:
			self.lead_location = frappe.db.get_value("Lead", self.lead, "lead_geolocation")
	...
	def validate(self):
		...
		target = parse_geo(self.lead_location)
